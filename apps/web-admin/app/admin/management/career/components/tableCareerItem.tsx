...
import { TranslationCareerDto } from '@/admin/packages/types/career';

interface ItemDto {
    careerItem: CareerSchema;
    index: number;
    filter: FilterDto;
    myData: MyDataDto;
    refetch: () => void;
...
function TableCareerItemComponent({ careerItem, index, filter, myData, refetch, setOpen }: ItemDto) {

    const [openDelete, setOpenDelete] = useState(false);
    const [myRole, setMyRole] = useState("VIEWER" as StaffRoleDto);
    const [translations, setTranslations] = useState({} as TranslationCareerDto);

    useEffect(() => {
        if (careerItem && myData) {
            setMyRole(myData.role);
            const translation = careerItem?.translationCareers?.find(tr => tr.local === "en");
            if (translation) {
    ...
    const removeStaffByIdMutation = trpc.app.admin.manage.career.removeOneById.useMutation({
        onSuccess,
        onError,
    });
    ...
    return (
        <>
            <TableRow key={careerItem.id} className="w-auto">
                ...
                <TableCell>
                    <div className="flex items-center gap-3">
                        ...
                        >
                            {getInitials(translations.jobTitle ?? '')}
                        </div>
                        <div>
                            <p className="font-medium truncate w-md">{translations?.jobTitle ?? ''}</p>
                            <p className="font-medium truncate w-md">{translations?.description ?? ''}</p>
                        </div>
                    </div>
                </TableCell>
                ...
                <TableCell className="text-muted-foreground">
                    {GlobalHelper.formatDate(careerItem.createdAt as Date)}
                </TableCell>
                    ...
                    {canInteract ? (
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                {/* EDIT */}
                                {canEdit && <>
                                    <Link href={`/admin/management/faq/edit?id=${careerItem.id}`}>
                                        <DropdownMenuItem
                                            onClick={() => {
                                                setOpen(true);
                                            }}
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <span className="font-medium text-red-600 truncate w-sm">{translations?.jobTitle ?? ''}</span> from
                            the system.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
...
export default TableCareerItemComponent;
